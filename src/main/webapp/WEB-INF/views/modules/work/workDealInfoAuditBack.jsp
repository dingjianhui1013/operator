...
<body>
	<div style="display: none">
		<object id="ukeyadmin"
			codeBase="itrusukeyadmin.cab#version=3,1,13,705"
				...
				<tbody id="financeTd">
					<c:forEach items="${fpir }" var="list">
						<tr id="f+${list.sn}">
							...
							<td><fmt:formatDate
									value="${list.financePaymentInfo.createDate }"
									pattern="yyyy-MM-dd HH:mm:ss" /></td>
							<td><c:if
									test="${list.financePaymentInfo.paymentMethod==1 }">现金</c:if> <c:if
									test="${list.financePaymentInfo.paymentMethod==2 }">POS收款</c:if>
					...
					<tbody>
						...
						<tr>
							...
							<td><input type="checkbox" value="1" name="methodPos"
								...
								<c:if test="${workDealInfo.workPayInfo.methodPos }">checked="checked"</c:if>>POS收款
								<input type="text" name="posMoney" id="pos"
								onchange="fixMoney()" readonly="readonly"
								value="${workDealInfo.workPayInfo.posMoney }"></td>
							<td></td>
						</tr>
						<tr>
							<td><input type="checkbox" value="1" name="methodGov"
								onclick="choose(this)" id="gc" disabled="disabled"
								<c:if test="${agent.tempStyle == 1 }">checked="checked"</c:if>>标准：
								<c:if test="${agent.tempStyle == 1 }">${agent.tempName}</c:if></td>
							<td><input type="checkbox" value="1" name="methodGov"
								onclick="choose(this)" id="gc" disabled="disabled"
								<c:if test="${agent.tempStyle == 2 }">checked="checked"</c:if>>政府统一采购：
								<c:if test="${agent.tempStyle == 2 }">${agent.tempName}</c:if></td>
							<td><input type="checkbox" value="1" name="methodContract"
								onclick="choose(this)" id="cc" disabled="disabled"
								<c:if test="${agent.tempStyle == 3 }">checked="checked"</c:if>>合同采购：
								<c:if test="${agent.tempStyle == 3 }">${agent.tempName}</c:if></td>
							<td><input type="hidden" id="officeNameHid"
								value="${officeName }"> <input type="hidden"
								id="userNameHid" value="${userName }"> <input
								type="hidden" id="dateHid" value="${date }"> <input
						...
						<tr>
							...
							<td><input type="text" id="sumMoney" name="workTotalMoney"
								disabled="disabled"
								value="${type0+type1+type2+type3+type4+type5+type6 }"></td>
							<td>已收发票</td>
						...
						<tr>
							<c:if test="${bgType }">
								<td>变更缴费类型为：</td>
								<td>计费策略类型： <select id="agentId" name="agentId">
										<option value="0">请选择</option>
								</select> <br> 计费策略模版： <select id="agentDetailId"
									name="agentDetailId">
										<option value="0">请选择</option>
								</select> <!-- <input type="radio" id="gov" name="payType" checked="checked">政府统一采购
						...
						<c:if test="${revoke }">
							<tr>
								...
								<td><input type="checkbox" id="tKey"
									<c:if test="${!tKey }">disabled</c:if> checked="checked"
									onclick="changeInputStatus(this)">退还key</td>
								<td>CSP:<select name="provider" id="provider"></select> <br>
									业务key序列号：<input type="text" readonly="readonly"
									value="${workDealInfo.keySn }"><br> 当前key序列号：<input
									type="text" id="keySn" name="keySn" readonly="readonly">
								</td>
								<td><input type="button" class="btn btn-primary"
									value="检测key" id="keyButton" style="display: none"
									onclick="javascript:selectKeyNum()" /> <input type="hidden"
									id="isSame" /></td>
							</tr>
						</c:if>
					</tbody>
	...
	<div class="form-actions"
		...
		<c:if test="${revoke }">
			<input class="btn btn-primary" type="button" id="tf" value="退费"
				onclick="javascript:buttonFrom();" />&nbsp;
				</c:if>

		<c:if test="${!revoke}">
			<input class="btn btn-primary" type="button" value="变更"
				onclick="javascript:checkAgent();" />&nbsp;
