...
<body>
	...
	<form:form id="searchForm" modelAttribute="workDealInfo"
		action="${ctx}/work/workDealInfo/StatisticalDealPayList"
		...
		<label>付款方式 ：</label>
			<select name="payMethod" id="payMethod">
				<option value="0"  <c:if test="${payMethod=='0'}"> selected="selected" </c:if> >全部</option>	
				...
				<option value="5"  <c:if test="${payMethod=='5'}"> selected="selected" </c:if>>政府统一采购</option>
				<option value="6"  <c:if test="${payMethod=='6'}"> selected="selected" </c:if>>合同采购</option>
			</select> 
		</div>
		...
		<tbody>
			<c:forEach items="${page.list}" var="configDealPay">
				<tr>
					...
					<td>
					<fmt:formatDate value="${configDealPay.workPayInfo.createDate}" pattern="yyyy-MM-dd HH:mm:ss"/>
					</td>
					<td>
						<c:choose>
