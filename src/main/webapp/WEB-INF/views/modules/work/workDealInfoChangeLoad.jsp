...
<body>
	...
	<form:form id="inputForm"
		action="${ctx}/work/workPayInfo/changeSavePayInfo" method="POST"
						...
						<tbody>
							...
							<tr>
								...
								<td><c:if test="${chargeAgent.chargeMethodPos==true }"><input type="checkbox" value="1" name="methodPos" id="pc"
									...
									type="text" name="posMoney" id="pos"
									readonly="readonly"  value="0"></c:if><c:if test="${chargeAgent.chargeMethodPos!=true }"><input type="checkbox" disabled="disabled">pos收款<input
									type="text" disabled="disabled" value="0"><input
									type="hidden" name="posMoney" id="pos"
									readonly="readonly"  value="0">
									</c:if></td>
								<td><c:if test="${chargeAgent.chargeMethodBank==true }"><input type="checkbox" value="1" name="methodBank" id="bc"
									onclick="changeInputStatus(this)">银行转账<input
									type="text" name="bankMoney" id="bank"
									readonly="readonly"  value="0"></c:if> <c:if test="${chargeAgent.chargeMethodBank!=true }"><input type="checkbox" disabled="disabled">银行转帐<input
									type="text" disabled="disabled" value="0"><input
									type="hidden" name="bankMoney" id="bank"
									readonly="readonly"  value="0"></c:if></td>
								<td><c:if test="${chargeAgent.chargeMethodAlipay==true }"><input type="checkbox" value="1" name="methodAlipay" id="ac"
									onclick="changeInputStatus(this)">支付宝转账<input
									type="text" name="alipayMoney" id="alipay"
									 readonly="readonly" 
									value="0"></c:if> <c:if test="${chargeAgent.chargeMethodAlipay!=true }"><input type="checkbox" disabled="disabled">支付宝转账<input
									type="text" disabled="disabled" value="0"><input
									type="hidden" name="alipayMoney" id="alipay"
									 readonly="readonly" 
									value="0"></c:if></td>
								</tr>
							<tr>
								<td><c:if test="${chargeAgent.chargeMethodGov==true }"><input type="checkbox" value="1" name="methodGov" onclick="choose(this)" id="gc" methodval="0"
									>政府统一采购</c:if><c:if test="${chargeAgent.chargeMethodGov!=true }"><input type="checkbox" disabled="disabled"
									>政府统一采购</c:if></td>
								<td><c:if test="${chargeAgent.chargeMethodContract==true }"><input type="checkbox" value="1" name="methodContract" onclick="choose(this)" id="cc" methodval="1"
									>合同采购</c:if><c:if test="${chargeAgent.chargeMethodContract!=true }"><input type="checkbox" 
									>合同采购</c:if></td>
									<td>
							...
							<tr>
								<td>业务应收合计:</td>
								<td><input type="text" id="sumMoney" name="workTotalMoney" disabled="disabled" value="<c:if test='${empty addMoney}'>${type0+type1+type2+type3+type4+type5+type6 }</c:if><c:if test='${!empty addMoney}'>${addMoney }</c:if>"></td>
								<td><!-- 本次应收金额 -->
		...
		<div class="form-actions" style="text-align: center;">
			<shiro:hasPermission name="work:workDealInfo:edit">
				<input id="btnSubmit" class="btn btn-primary" type="submit"
					onclick="return onSubmit()" value="提交" />&nbsp;</shiro:hasPermission>
