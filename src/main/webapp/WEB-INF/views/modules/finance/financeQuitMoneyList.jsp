...
<script type="text/javascript">
	...
	function quitMoney(id, residueMoney){
		...
		top.$.jBox(html, {title : "退费信息", buttons : {"确定" : "ok", "关闭" : "close"}, submit : submit});
	}
		...
		<tbody>
			<c:forEach items="${page.list }" var="financeQuitMoney" varStatus="f">
				<tr>
					...
					<c:if test="${financeQuitMoney.workDealInfo ==null }">
					<fmt:formatNumber value="${financeQuitMoney.financePaymentInfo.paymentMoney}" type="number" />
					</c:if>
					</td>
					...
					<c:if test="${financeQuitMoney.workDealInfo !=null }">
					<fmt:formatDate value="${financeQuitMoney.workDealInfo.workCertInfo.signDate}" pattern="yyyy-MM-dd" />
					</c:if>
					<c:if test="${financeQuitMoney.workDealInfo ==null }">
					<fmt:formatDate value="${financeQuitMoney.financePaymentInfo.payDate}" pattern="yyyy-MM-dd" />
					</c:if>
					...
					<c:if test="${financeQuitMoney.workDealInfo !=null }">
						<c:if test="${financeQuitMoney.workDealInfo.workPayInfo.methodPos }">POS机付款&nbsp;</c:if>
						<c:if test="${financeQuitMoney.workDealInfo.workPayInfo.methodMoney }">现金交易&nbsp;</c:if>
						<c:if test="${financeQuitMoney.workDealInfo.workPayInfo.methodAlipay }">支付宝交易&nbsp;</c:if>
						<c:if test="${financeQuitMoney.workDealInfo.workPayInfo.methodGov }">政府统一采购&nbsp;</c:if>
						<c:if test="${financeQuitMoney.workDealInfo.workPayInfo.methodContract }">合同采购&nbsp;</c:if>
						<c:if test="${financeQuitMoney.workDealInfo.workPayInfo.methodBank }">银行转账&nbsp;</c:if>					
					</c:if>
					<c:if test="${financeQuitMoney.workDealInfo ==null }">
					...
					<td>
						<c:if test="${financeQuitMoney.status==1 }">未处理</c:if>
						<c:if test="${financeQuitMoney.status==2 }">已处理</c:if>
						<c:if test="${financeQuitMoney.status==null }">已完成</c:if>
					
					</td>
					<td>
						<c:if test="${financeQuitMoney.status==1 }">
						<a href="javascript:quitMoney(${financeQuitMoney.id }, ${financeQuitMoney.workDealInfo.workPayInfo.workTotalMoney})">确认退费</a>
