...
<script type="text/javascript">
	...
	function dca() {
		...
		window.location.href = "${ctx }/settle/keySettle/export?supplierId="
				+ supplierId + "&keyId=" + keyId + "&startTime=" + startTime
				+ "&endTime=" + endTime;
	...
	function addGene() {
		...
		$.getJSON(url + supplierId + "&_=" + new Date().getTime(), function(
				data) {
			var html = "";
			//console.log(data);
...
<body>
	...
	<form:form id="searchForm" action="${ctx}/settle/keySettle"
		method="post" class="breadcrumb form-search">
		<input id="pageNo" name="pageNo" type="hidden" value="${page.pageNo}" />
		<input id="pageSize" name="pageSize" type="hidden"
			value="${page.pageSize}" />
		<div>
			<label>KEY 厂商：</label> <select name="supplierId" id="supplierId"
				onchange="addGene()">
			...
			</select> <label>KEY编码：</label> <input type="text" name="keySn" id="keySn"
				value="${keySn}" />
		</div>
		<br />
		<label>入库时间：</label>
		...
		<tbody>
			...
			<c:forEach items="${page.list}" var="keyPurchase" varStatus="status">
				...
				<tr>
					...
					<td><fmt:formatDate value="${keyPurchase.storageDate }"
					...
					<td><c:if test="${keyPurchase.status==1}">已付</c:if> <c:if
							test="${keyPurchase.status==0}">未付</c:if> <c:if
							test="${keyPurchase.status==null}">状态位置</c:if></td>
