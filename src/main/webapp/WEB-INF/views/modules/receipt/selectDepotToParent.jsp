	...
	<script type="text/javascript">
		...
		var pre_selectedNodes =[
		        <c:forEach items="${selectedList}" var="depot">
  		        	{id:"${depot.id}",
		  	         pId:"0",
		           	 name:"<font color='red' style='font-weight:bold;'>${depot.receiptName}</font>"},
		           </c:forEach>];
		...
		function clearAssign(){
			var submit = function (v, h, f) {
			    if (v == 'ok'){
					...
					if(pre_ids.sort().toString() == ids.sort().toString()){
						tips = "未给库房【${depot.receiptName}】分配下级库房！";
					}else{
						tips = "已选库房清除成功！";
					}
					ids=pre_ids.slice(0);
			...
			tips="确定清除库房【${depot.receiptName}】下的已选库房？";
