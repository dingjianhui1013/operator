...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/signature/signatureConfigChargeAgent/getSignatureChargeAgentList">签章计费策略模板列表</a></li>
		<li><a href="${ctx}/signature/signatureConfigChargeAgent/form">签章计费策略模板添加</a></li>
	</ul>
	<form:form id="searchForm" modelAttribute="signatureConfigChargeAgent" action="${ctx}/signature/signatureConfigChargeAgent/getSignatureChargeAgentList" method="post" class="breadcrumb form-search">
		<input id="pageNo" name="pageNo" type="hidden" value="${page.pageNo}"/>
		<input id="pageSize" name="pageSize" type="hidden" value="${page.pageSize}"/>
		<label>签章计费策略模板名称 ：</label><form:input path="tempName" htmlEscape="false" maxlength="50" class="input-medium"/>
		&nbsp;
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<th>序号</th>
		<th>签章计费策略模板名称</th>
		<shiro:hasPermission name="signature:signatureConfigChargeAgent:edit"><th>操作</th></shiro:hasPermission>
		</tr></thead>
		<tbody>
		<c:forEach items="${page.list}" var="signatureConfigChargeAgent" varStatus="config">
			<tr>
				...
				<shiro:hasPermission name="signature:signatureConfigChargeAgent:edit"><td>
					<a href="${ctx}/signature/signatureConfigChargeAgent/form?id=${signatureConfigChargeAgent.id}">修改</a>
