...
//			String sql ="select  T.DDATE FROM( select  distinct(TO_CHAR(k.CREATE_DATE,'yyyy-MM-dd')) as DDATE from RECEIPT_ENTER_INFO k , RECEIPT_DEPOT_INFO d  where  k.DEPOT_ID = d.ID AND d.office_id = ?  AND k.CREATE_DATE >= TO_DATE(?, 'yyyy-MM-dd')-"+days+" AND k.CREATE_DATE < TO_DATE(?, 'yyyy-MM-dd')) t "
//					+" where t.DDATE NOT IN( select  distinct(TO_CHAR(S.STATISTIC_DATE,'yyyy-MM-dd')) from STATISTIC_SEAL_DAY_DATA s where s.STATISTIC_DATE >= TO_DATE( ? , 'yyyy-MM-dd')-"+days+" and s.OFFICE_ID= ?) ORDER BY T.DDATE ASC";
//			List<String> list = statisticSealDayDataDao.findBySql(sql, officeId,countDate,countDate,countDate,officeId);
//			
//			if(list.size()>0){
...
//				e.printStackTrace();T
//			}
//			List<StatisticDayData> 	statisticDayDataList = statisticSealDayDataDao.find(dc);
			return list;
		}
