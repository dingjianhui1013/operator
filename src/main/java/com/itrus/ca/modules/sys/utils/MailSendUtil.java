...
public class MailSendUtil {
	...
	public boolean sendMail(String to,String title,String message,boolean ssl,String smtp,String user,String pass,String port,String ccAddress){
		try {
			if (ssl) {
				...
				String fromUser = user+"@"+ss[ss.length-3]+"."+ss[ss.length-2]+"."+ss[ss.length-1];
	 ...
	 * @return
	 */
	public boolean testMailHostIsValid(String to,boolean ssl,String smtp,String user,String pass,String port){
		String server = new String(smtp);
		String[] ss = server.split("\\.");
		String fromUser = user+"@"+ss[ss.length-3]+"."+ss[ss.length-2]+"."+ss[ss.length-1];
