		...
		if(!isInit){
			...
			if(sysCrlContexts!=null && sysCrlContexts.size()>0) {
				for (SysCrlContext sysCrlContext : sysCrlContexts) {
					cvm.addSupportCA(X509Certificate.getInstance(sysCrlContext.getCaCertBuf()),null, sysCrlContext.getCrlUrl(), sysCrlContext.getRetryPolicy(), sysCrlContext.getCheckCrl(),true);
				}
				isInit = true;
				return true;
			}
