 ...
 *
 */
public class FixFirstSNThread4 implements Runnable {
	...
	public FixFirstSNThread4(List<String> all) {
		this.all = all;
	}
	...
	private class Inner implements Runnable {
		...
		public synchronized void run() {
			try {
				workDealInfoService.fixFirstCertSNByError4(l);
			} catch (Exception ex) {
				// 事务问题，可以不显示
			}
		}
