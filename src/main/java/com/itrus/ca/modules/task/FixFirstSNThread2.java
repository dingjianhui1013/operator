 ...
 *
 */
public class FixFirstSNThread2 implements Runnable {
	...
	public FixFirstSNThread2(List<String> all) {
		this.all = all;
	}
	...
	private class Inner implements Runnable {
		...
		public synchronized void run() {
			try {
				workDealInfoService.fixFirstCertSNByError2(l);
			} catch (Exception ex) {
				// 事务问题，可以不显示
			}
		}
